...
export const sendPasswordResetEmail = async (email, name, code) => {
    ...
    await transporter.sendMail({
        from: `"Magari Zetu" <${process.env.EMAIL_USER}>`,
        to: email,
        subject: 'Reset your Magari Zetu password',
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 2rem; background: #f8fafc;">
                <h2 style="color: #0D3B66;">Password Reset Request</h2>
                <p style="color: #333;">Hi ${name}, use this code to reset your password:</p>
                <div style="background: #0D3B66; color: white; font-size: 2rem; font-weight: bold; text-align: center; padding: 1rem; border-radius: 8px; letter-spacing: 0.3em; margin: 1.5rem 0;">
                    ${code}
                </div>
                <p style="color: #777; font-size: 0.9rem;">This code expires in 15 minutes. If you did not request a password reset you can ignore this email.</p>
            </div>
        `
    })
}